...
window.ActiveXObject&&!window.CanvasRenderingContext2D && function(j, m) {
	...
	function aa(a) {
		var b = parseInt(a.width, 10), c = parseInt(a.height,
		10);
		if (isNaN(b) || b < 0)
			b = 300;
		if (isNaN(c) || c < 0)
	...
	var k = null, u = "canvas", X = "CanvasRenderingContext2D", Y = "CanvasGradient", Z = "CanvasPattern", K = "FlashCanvas", $ = "G_vmlCanvasManager", V = "onfocus", I = "onmousedown", J = "onmouseup", W = "onpropertychange", Q = "onreadystatechange", U = "onunload", q;
	try {
		q = (new ActiveXObject("ShockwaveFlash.ShockwaveFlash")).GetVariable("$version").match(/[\d,]+/)[0].replace(/,/g, ".")
	} catch (ia) {
		q = 0
	}
	var r = j[K + "Options"] || {}, ba = r.swfPath || ga().replace(/[^\/]+$/, ""), x;
	...
	D.prototype = {
		...
		createPattern: function(a, b) {
			a || g(17);
			var c = a.tagName, d, e, f, h = this.f;
			if (c) {
				c = c.toLowerCase();
				if (c === "img")
		...
		drawImage: function(a, b, c, d, e, f, h, i, s) {
			a || g(17);
			var o = a.tagName, t, p, M, w = arguments.length, N = this.f;
			if (o) {
				o = o.toLowerCase();
				if (o === "img")
					t = a.getAttribute("src", 2);
				else if (o === u) {
					p = this.G(a);
					M = a !== this.canvas
				} else if (o === "video")
			...
			this.e(0);
			if (w === 3)
				this.a.push("u", w, p, b, c);
			else if (w === 5)
				this.a.push("u", w, p, b, c, d, e);
			else if (w === 9) {
				if (d === 0 || e === 0)
					g(1);
		...
		putImageData: function(a, b, c, d, e, f, h) {
			...
			var i = arguments.length, s = a.width, o = a.height, t = a.data;
			if (i === 3)
				this.a.push("x", i, s, o, t.toString(), b, c);
			else if (i === 7) {
				isFinite(d) &&
				isFinite(e) && isFinite(f) && isFinite(h) || g(9);
		...
		e: function(a) {
			var b = this.a, c;
			if (this.l !== this.globalAlpha)
				b.push("I", this.l = this.globalAlpha);
			if (this.m !== this.globalCompositeOperation)
				b.push("J", this.m = this.globalCompositeOperation);
			if (this.t !== this.shadowOffsetX)
				b.push("T", this.t = this.shadowOffsetX);
			if (this.u !== this.shadowOffsetY)
				b.push("U", this.u = this.shadowOffsetY);
			if (this.r !== this.shadowBlur)
				b.push("V",
			...
			if (a & 1)
				if (this.k !== this.fillStyle) {
					c = this.k = this.fillStyle;
					if (typeof c === "object")
						c = c.id;
					else ("" + c)
						.indexOf("%") > 0 && this.i.push(b.length + 1);
						b.push("L", c)
				}
			if (a & 2)
				if (this.v !== this.strokeStyle) {
			...
			if (a & 4) {
				if (this.p !== this.lineWidth)
					b.push("P", this.p =
					this.lineWidth);
				if (this.n !== this.lineCap)
					b.push("Q", this.n = this.lineCap);
				if (this.o !== this.lineJoin)
					b.push("R", this.o = this.lineJoin);
				if (this.q !== this.miterLimit)
					b.push("S", this.q = this.miterLimit)
			}
			if (a & 8) {
				if (this.j !== this.font) {
				...
				if (this.z !== this.textAlign)
					b.push("p", this.z = this.textAlign);
				if (this.A !== this.textBaseline)
					b.push("q", this.A = this.textBaseline);
				if (this.H !== this.canvas.currentStyle.direction)
					b.push("1", this.H = this.canvas.currentStyle.direction)
			}
		},
		d: function() {
			var a = this;
		...
		g: function(a) {
			var b, c, d, e = this.i, f = this.h, h = this.a, i = this.w;
			if (h.length) {
				this.b && this.Q();
				if (a) {
					b = 0;
				...
				if (a) {
					...
					return i.CallFunction('<invoke name="executeCommand" returntype="javascript"><arguments><string>' + b + "</string></arguments></invoke>")
			}
		},
		P: function(a, b) {
			this.g();
		...
		F: function(a) {
			var b = this.f, c = B[b], d = ca[b], e = c[a] = L[b]++;
			if (e >= l.N-1)
				L[b] = 0;
			e in d && delete c[d[e]];
	...
	var ea = {
		...
		initElement: function(a) {
			...
			if (m.compatMode === "BackCompat" ||!j.XMLHttpRequest)
				C[b].style.overflow = "hidden";
			var h = new D(a, d);
		...
		setOptions: function(a) {
			for (var b in a) {
				...
				case "disableContextMenu":
					...
					c = void 0;
					for (c in n) {
						var e = n[c], f = d ? "attachEvent": "detachEvent";
						e.firstChild[f](I, G);
						e[f](J, H)
					}
					break;
	...
	j.attachEvent(U, T);
	if (x.indexOf(location.protocol +
	"//" + location.host + "/") === 0) {
